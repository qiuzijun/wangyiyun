      ...
      <div class="songText" v-if="songListInformation">
        ...
        <div class="g_d_content">
          ...
          <div class="establishName">
            ...
            />
            <div class="e_name">
              <span class="e_n">{{ list.playlist.creator.nickname }}</span>
              <span>{{ list.playlist.Time }} 创建</span>
            </div>
          </div>
          <div class="imgList">
            <div class="b_f">
              <div class="b_f_left" @click="playList(songListId)">
                <p>播放</p>
              </div>
            ...
            <div class="s_c">
              ...
              <p>({{ list.playlist.subscribedCount }})</p>
            </div>
            <div class="z_fa">
            ...
            <div class="xin_x">
              ...
              <p>({{ list.playlist.commentCount }})</p>
            </div>
          </div>
          <div class="label">
            <span>标签:</span>
            <ul>
              <li v-for="(item, index) in list.playlist.tags" :key="index">
                <span>{{ item }}</span>
              </li>
            </ul>
          </div>
          <div class="introduce">
            <span>介绍:</span>
            <span>{{ list.playlist.description }}</span>
          </div>
        </div>
        <div class="list">
          <div class="listName">
            <h3>歌曲列表</h3>
            <p class="ff_p">{{ list.playlist.tracks.length }}首歌</p>
            <p>
              播放：<span>{{ list.playlist.playCount }}</span
              >次
            </p>
          </div>
          <div class="listContent">
            ...
            <div class="musicList">
              <ul>
                <li
                  v-for="(item, index) in list.playlist.tracks"
                  :key="item.id"
                >
                  <div class="number">
                    <p>
                  ...
                  <div class="album">
                    <p @click="routeDisc(item.al.id)">{{ item.al.name }}</p>
                  </div>
                </li>
              </ul>
            </div>
...
<script>
import { comment, soaringList } from "@/api/http.js";
export default {
  data() {
    return {
      // 评论
  ...
  methods: {
    ...
    async getPlaylist() {
      ...
      res.data.playlist.Time = this.$moment(
  ...
  created() {
    if (this.$route.params.id == undefined) {
      ...
      //   if (localStorage.getItem("songListId") == this.$route.params.singer.id) {
      //     this.songListId = localStorage.getItem("songListId");
      //   } else {
